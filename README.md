...
### 🔐 Система авторизации модераторов (НОВОЕ!)
- **Автоматическая авторизация** - система проверяет модераторов в листе "Пользователи" Google Sheets
...
- **Ручная авторизация** - при отсутствии в системе открывается модальное окно для ввода данных
- **Валидация и форматирование** - автоматическое форматирование статика в модальном окне
- **Предупреждения** - пользователь получает информацию о необходимости авторизации у администратора
- **Надёжность** - система никогда не прерывает работу, использует запасные варианты
- **Интеграция** - работает в формах увольнения и назначения ролей
- **Аудит записи** - все записи в аудит используют корректные данные модератора

### 🔐 Упрощенная система авторизации модераторов
- **Автоматическая регистрация** - новые модераторы регистрируются мгновенно
- **Форма авторизации** - Email, Имя Фамилия, Статик (авто-формат), Должность
- **Автоматический доступ к Google Sheets** - модераторы сразу получают права редактора
- **Валидация данных** - проверка корректности email и статика
...
- **Упрощенный процесс** - без ручного вмешательства администратора
- **Мгновенная работа** - модератор может сразу использовать систему