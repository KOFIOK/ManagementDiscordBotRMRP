...
Для работы бота необходимо создать в [Discord Developer Portal](#1-создание-приложения) приложение и бота, а затем указать токен:
...
POSTGRES_PORT=5432
POSTGRES_DB=army_bot_db
POSTGRES_USER=postgres
POSTGRES_PASSWORD=simplepassword

# Optional: Database URL (overrides individual settings if provided)
DATABASE_URL=postgresql+asyncpg://postgres:simplepassword@127.0.0.1:5432/army_bot_db
```
В нём на данный момент нас интересует строчка DISCORD_TOKEN, куда и нужно указать токен бота (пример: `BDSB4NKSdsklaLN7dha.sdjLAWNdnss6sdl2kalfhd.SE.FnGB890qbnFHFS`)
...
### В разработке...
> sql-скрипт с готовой структурой Базы Данных и сами данные будут в закрытом доступе, разработать базу данных вы должны сами, опираясь на utils/database/models.py и физическую модель, представленную ниже.

### Информация по PostgreSQL и установке

Для детальной информации по структуре базы, инструментам миграции и пошаговым инструкциям см. руководство по установке PostgreSQL: [postgresql_migration_guide.md](postgresql_migration_guide.md).  
В нём описаны схемы таблиц, утилиты проверки (db_schema_check.py), примеры SQL (пока отсутствуют) и рекомендации по валидации после переноса данных.
...
- `/reload_config` - перезагрузка настроек без перезапуска (не рекомендуется!)
