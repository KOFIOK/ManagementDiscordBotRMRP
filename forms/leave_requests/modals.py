...
class LeaveRequestModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(
                title="✅ Заявка на отгул подана успешно!",
                description="Ваша заявка отправлена руководству для рассмотрения.",
            ...
            embed.add_field(
                name="📋 Детали заявки:",
                value=(
                    f"**Время:** {start_time} - {end_time} ({duration_text})\n"
                    f"**Статус:** Ожидает рассмотрения\n"
                    f"**Подразделение:** {department.upper()}"
                ),
                inline=False
            )
    ...
    async def _send_request_to_channel(self, interaction, request_id, name, static, 
        ...
        # Get ping roles
        ping_roles = LeaveRequestDepartmentDetector.get_ping_roles(department, interaction.guild)
