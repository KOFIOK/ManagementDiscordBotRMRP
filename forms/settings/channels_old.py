...
class ChannelConfigSelect(ui.Select):
    ...
    async def show_channel_selection(self, interaction: discord.Interaction, config_type: str):
        ...
        if config_type == "role_assignment":
            ...
            await self.show_warehouse_config(interaction)
        else:
...
class MedicalRoleModal(BaseSettingsModal):
    ...
    async def show_warehouse_config(self, interaction: discord.Interaction):
        ...
        if not limits_status:
            limits_text = "🚨 Все лимиты отключены"
        else:
...
class WarehouseChannelsConfigView(BaseSettingsView):
    ...
    async def set_request_channel(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Set warehouse request channel"""
        modal = WarehouseChannelSelectionModal("warehouse_request_channel", "📦 Канал запросов склада")
        await interaction.response.send_modal(modal)
    ...
    async def set_audit_channel(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Set warehouse audit channel"""
        modal = WarehouseChannelSelectionModal("warehouse_audit_channel", "📊 Канал аудита склада")
        await interaction.response.send_modal(modal)
...
class WarehouseChannelSelectionModal(BaseSettingsModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            channel = ChannelParser.parse_channel(self.channel_input.value, interaction.guild)
            
            if not channel:
                embed = discord.Embed(
                    title="❌ Канал не найден",
                    description="Указанный канал не найден. Проверьте правильность ввода.",
                    color=discord.Color.red()
                )
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return
            ...
            # Setup channel if it's request channel
            if self.config_key == "warehouse_request_channel":
                try:
                    ...
                    setup_message = " Закрепленное сообщение с категориями склада создано."
                except Exception as e:
                    print(f"Ошибка при создании сообщения склада: {e}")
                    setup_message = " Канал настроен, но возникла ошибка при создании сообщения."
            else:
                setup_message = ""
            
            embed = discord.Embed(
                title="✅ Канал настроен",
                description=f"Канал установлен: {channel.mention}{setup_message}",
                color=discord.Color.green()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            ...
            embed = discord.Embed(
                title="❌ Ошибка",
                description=f"Произошла ошибка при настройке канала: {str(e)}",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
