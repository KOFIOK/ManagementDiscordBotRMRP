...
class WarehousePersistentRequestView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ Выдать склад", style=discord.ButtonStyle.green, custom_id="warehouse_approve")
    async def approve_request(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            try:
                from forms.warehouse_audit import create_automatic_audit_from_approval
    ...
    @discord.ui.button(label="❌ Отклонить", style=discord.ButtonStyle.red, custom_id="warehouse_reject")
    ...
    @discord.ui.button(label="🗑️ Удалить запрос", style=discord.ButtonStyle.secondary, custom_id="warehouse_delete")
...
class WarehousePersistentMultiRequestView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ Выдать склад", style=discord.ButtonStyle.green, custom_id="warehouse_multi_approve")
    ...
    @discord.ui.button(label="❌ Отклонить заявку", style=discord.ButtonStyle.red, custom_id="warehouse_multi_reject")
    ...
    @discord.ui.button(label="🗑️ Удалить запрос", style=discord.ButtonStyle.secondary, custom_id="warehouse_multi_delete")
...
class WarehouseItemSelectView(discord.ui.View):
    ...
    def __init__(self, category: str, category_info: Dict, warehouse_manager: WarehouseManager):
        super().__init__(timeout=300)  # 5 минут таймаут для выбора предмета
        self.category = category
        ...
        for i, item in enumerate(items):
            if i < 20:  # Максимум 20 кнопок (4 ряда по 5)
                button = discord.ui.Button(
                    ...
                    custom_id=f"warehouse_item_temp_{i}",  # Временный ID
...
class WarehouseCartView(discord.ui.View):
    """View для управления корзиной запросов - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    
    def __init__(self, cart: WarehouseRequestCart, warehouse_manager: WarehouseManager, is_submitted: bool = False):
        super().__init__(timeout=300)  # 5 минут на работу с корзиной
        self.cart = cart
        self.warehouse_manager = warehouse_manager
...
class WarehouseFinalDetailsModal(discord.ui.Modal):
    ...
    def __init__(self, cart: WarehouseRequestCart, warehouse_manager: WarehouseManager, interaction_original: discord.Interaction):
    ...
    def create_with_prefilled_data(cls, cart: WarehouseRequestCart, warehouse_manager: WarehouseManager, 
                                 interaction_original: discord.Interaction, name: str = "", static: str = ""):
        """
    ...
    async def _process_warehouse_request_background(self, interaction: discord.Interaction, name: str, static: str):
        ...
        try:
            ...
            for item in self.cart.items:
                item.user_name = name
                item.user_static = static            # Отправляем заявку (самая тяжелая операция)
