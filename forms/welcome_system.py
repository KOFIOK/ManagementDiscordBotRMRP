...
class WelcomeSystem:
    ...
    async def send_welcome_message(member: discord.Member):
        ...
        try:
            ...
            # Создаем основное приветственное embed сообщение
            embed = discord.Embed(
                title=get_private_messages(member.guild.id, "welcome.title", "🎖️ Добро пожаловать в нашу Фракцию!"),
            ...
            embed.add_field(
                ...
                value=get_private_messages(member.guild.id, "welcome.description", "").format(step_text=step_text),
                inline=False
            )

            embed.add_field(
                name="👤 Уже являетесь госслужащим?",
                value=get_private_messages(member.guild.id, "welcome.existing_member_info", ""),
                inline=False
            )

            # Создаем embed с информацией о военном билете
            embed_ticket = discord.Embed(
                title=get_private_messages(member.guild.id, "welcome.military_ticket_title", "🎟️ Как получить военный билет?"),
                color=0x00FF00,  # Зеленый цвет
                timestamp=discord.utils.utcnow()
            )
            # Добавляем поля с информацией
            embed_ticket.add_field(
                name="",
                value=get_private_messages(member.guild.id, "welcome.military_ticket_info", ""),
                inline=False
            )

            embed_ticket.set_footer(
                text=get_private_messages(member.guild.id, "welcome.footer", "Желаем успешной службы! | Руководство ВС РФ"),
