"""
Скрипт для добавления пробела перед эмодзи в name= при создании/обновлении embed полей
Это необходимо для консистентности с сравнениями field.name == " emoji..."
"""
import re
...
# Паттерн для поиска name="emoji в add_field/set_field_at
# Ищем: name="emoji (БЕЗ пробела после кавычки)
EMBED_FIELD_PATTERN = re.compile(
    rf'(\bname\s*=\s*["\'])([{EMOJI_LIST}])',
    re.UNICODE
)
...
def fix_line(line: str) -> Tuple[str, bool]:
    ...
    """
    def replacer(match):
        name_eq = match.group(1)  # name="
        emoji = match.group(2)  # первый символ эмодзи
        return f"{name_eq} {emoji}"  # Добавляем пробел
    
    new_line, count = EMBED_FIELD_PATTERN.subn(replacer, line)
