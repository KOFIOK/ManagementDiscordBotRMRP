"""
Скрипт для удаления пробелов в field.name сравнениях
Изменяет: field.name == " 📊..." -> field.name == "📊..."
"""
import re
...
# Паттерн для поиска field.name == " emoji (с пробелом)
FIELD_NAME_PATTERN = re.compile(
    rf'(field\.name\s*==\s*["\'])\s+([{EMOJI_LIST}])',
    re.UNICODE
)

def fix_line(line: str) -> Tuple[str, bool]:
    """
    Исправляет строку, удаляя пробел после кавычки перед эмодзи
    ...
    def replacer(match):
        ...
        return f"{quote}{emoji}"  # Удаляем пробел
