    ...
    try:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• **Обработка заявок на получение ролей** - модерация военных/гражданских заявок\n"
                "• **Обработка заявлений в подразделения** - модерация поступления в части\n"
                "• **Обработка запросов склада** - выдача складского имущества\n"
                "• **Иерархическая модерация** - контроль согласно воинской иерархии\n"
        ...
        embed.add_field(
            ...
            f"3. **Начните модерировать** - все заявки обрабатываются через кнопки в интерфейсе"
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="❓ Узнайте больше",
            value="Введите команду `/help` для получения полной справки по системе",
            inline=False
        )
    ...
    try:
        ...
        embed.add_field(
            ...
            value=(
                ...
                "• **Команда `/reload_config`** - горячая перезагрузка конфигурации\n"
                "• **Команда `/migrate-ping-settings`** - миграция настроек пингов\n"
        ...
        embed.add_field(
            ...
            value=(
                "• **Каналы подразделений** - настройка заявлений в части\n"
                "• **Каналы системы** - увольнения, аудит, чёрный список, роли, медицина\n"
